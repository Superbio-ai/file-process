...
class H5ADFileProcessor(FileProcessorBase):
    ...
    def get_targets(self, var_targets: Optional[bool] = None):
        return list(self.adata.var.columns) if var_targets else list(self.adata.obs.columns)
    ...
    def get_preview(self, var_targets: Optional[bool] = None):
        target_names = self.get_targets(var_targets)
